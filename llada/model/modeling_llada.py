...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        # To support attention 1: 修改签名
        """
        Computes scaled dot product attention on query, key and value tensors, using an optional
        attention mask if passed, and applying dropout if a probability greater than 0.0 is specified.
        """

        # To support attention 2: 需要输出 attentions -> 强制 eager
        if self.config._attn_implementation == "eager":
            # 处理 GQA：repeat kv heads 到 q heads
        ...
        # To support attention 3: 保持原来的规则（添加输出部分的None）
    ...
    def attention(
        ...
        # To support attention 4: 签名修改成 attention(...) -> (attn_output, present, attn_weights)
        ...
        # shape: (B, nh, T, hs)
        # To support attention 5: 把 weights 往上层传
    ...
    def forward(
        ...
        # To support attention 6: 签名修改成 forward(...) -> (x, cache, attn_weights)
        raise NotImplementedError
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def forward(
        ...
        # To support attention 7: (x, cache, attn_weights)
        if self._activation_checkpoint_fn is not None:
...
class LLaDAOutput(NamedTuple):
    ...
    Each tensor is typically of shape (batch_size, num_heads, q_len, k_len).
    """
...
class LLaDABlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        return x, attn_key_values, all_attentions
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None, # To support attention 9
        replace_position: Optional[torch.Tensor] = None,
    ) -> LLaDAOutput:
        """
        ...
        self.config._attn_implementation = "flex"
        if output_attentions:
            self.config._attn_implementation = "eager"

        output_hidden_states = output_hidden_states if output_hidden_states is not None else False
        # To support attention 9
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache, attn_weights = block_group(
        ...
        return LLaDAOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None, attentions=tuple(all_attentions) if output_attentions else None,)  # type: ignore[arg-type]
