...
# Modified for lm-eval 0.4.11, which removes the get_dtype API.
def get_dtype(dtype: Union[str, torch.dtype]) -> torch.dtype:
    """Convert `dtype` from `str` to `torch.dtype` when possible."""
...
class Dream(LM):
    def __init__(
        ...
        L_min: Optional[int] = 8,
        L_max: Optional[int] = 128,
        lambda_u: Optional[float] = 1.2,
        topk_cache: Optional[int] = 64,
        ...
        **kwargs,
        ...
        self.topk_cache = topk_cache
        self.generated_token_num = 0
        self.generated_token_num_including_eos = 0
    ...
    def _generate_batch(self, prompts: List[str]) -> Tuple[List[str], Optional[List[int]]]:
        ...
        generation_ids, block_sizes = self.model.diffusion_generate(
            prompt_ids,
        ...
        self.generated_token_num += (generated_sequence != self.tokenizer.eos_token_id).sum().item()
        self.generated_token_num_including_eos += generated_sequence.numel()
        print(f"generated_token_num: {self.generated_token_num}")
        print(f"generated_token_num_including_eos: {self.generated_token_num_including_eos}")
        responses = [
        ...
        print(f"block sizes for each block: {block_sizes}")
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        if self.use_cache: # block generation w/ cache
            ...
            self.model.diffusion_generate = types.MethodType(BlockDreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(BlockDreamGenerationMixin._sample_parallel_depcap, self.model)
        else:
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            if nfe_history is not None:
                nfe_list.append(sum(nfe_history))
        ...
        print(f"Generated token num (including EOS): {self.generated_token_num_including_eos}")
        print(f"Generated token num per second: {self.generated_token_num / (end_time - start_time)}")
        print(f"Generated token num per second (including EOS): {self.generated_token_num_including_eos / (end_time - start_time)}")
        print()
        if nfe_list:
            print(f"NFE for each question (sum across all blocks): {nfe_list}")
            print(f"Total NFE across all questions: {sum(nfe_list)}")
            print(f"Average NFE per question: {sum(nfe_list) / len(nfe_list):.2f}")


        if self.save_dir is not None:
            metrics = {
                "total_tokens": int(self.generated_token_num),
                "total_time": float(end_time - start_time),
                "tokens_per_second": float(self.generated_token_num / (end_time - start_time)),
                "total_nfe": int(sum(nfe_list)),
            }
            metrics_path = os.path.join(self.save_dir, f"rank_{self.rank}_metrics.json")
    ...
    def suffix_greedy_prediction(self, prefix, target):
        ...
        if prefix_len > 0:
            seq[0, :prefix_len] = prefix.to(self.device)
        target = target.to(self.device)

        for _ in range(target_len):
            mask_index = seq == self.tokenizer.mask_token_id
            logits = self.get_logits(seq, prompt_index)[mask_index]
            pred_tokens = torch.argmax(logits, dim=-1)

            if pred_tokens.numel() > 1:
                log_probs = torch.log_softmax(logits.float(), dim=-1)
                confidence = torch.gather(log_probs, dim=-1, index=pred_tokens.unsqueeze(-1)).squeeze(-1)
                next_idx = torch.argmax(confidence)
                pred_tokens_masked = torch.full_like(pred_tokens, self.tokenizer.mask_token_id)
                pred_tokens_masked[next_idx] = pred_tokens[next_idx]
                pred_tokens = pred_tokens_masked

            seq[mask_index] = pred_tokens

        return torch.equal(seq[0, prefix_len:], target)
