...
from generate import (
    ...
    generate_adaptive_block_with_dual_cache,
    generate_adaptive_block_with_prefix_cache,
)
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        topk_cache: int = 128,            # store topK for baseline cache
    ...
    def generate_until(self, requests):
        ...
        if self.save_dir is not None:
            ...
            rank = self.rank
            save_path = os.path.join(self.save_dir, f'rank_{rank}_output.jsonl')
        ...
        for batch_idx, batch in enumerate(tqdm(batched_requests, desc="Generating...")):
            sample_i = processed_count + batch_idx
            # if sample_i == 1:
            #     break
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe, transfer_steps, block_sizes = generate_adaptive_block_with_dual_cache(self.model, input_ids, gen_length=self.gen_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, tau_low=self.tau_low, tau_high=self.tau_high, gamma=self.gamma, L_max=self.L_max, L_min=self.L_min, lambda_u=self.lambda_u, topk_cache=self.topk_cache)
                else:
                    generated_answer, nfe, transfer_steps, block_sizes = generate_adaptive_block_with_prefix_cache(self.model, input_ids, gen_length=self.gen_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, tau_low=self.tau_low, tau_high=self.tau_high, gamma=self.gamma, L_max=self.L_max, L_min=self.L_min, lambda_u=self.lambda_u, topk_cache=self.topk_cache)
            else:
                generated_answer, nfe, transfer_steps, block_sizes = generate_adaptive_block(self.model, input_ids, gen_length=self.gen_length, temperature=0, remasking=self.remasking, mask_id=self.mask_id, tau_low=self.tau_low, tau_high=self.tau_high, gamma=self.gamma, L_max=self.L_max, L_min=self.L_min, lambda_u=self.lambda_u, topk_cache=self.topk_cache)
            ...
            if self.is_instruct and 'task_id' in req.doc and str(req.doc['task_id']).lower().startswith('humaneval'):
                ...
                token_count = (generated_answer_ids != 126081).sum()
            ...
            if self.save_dir is not None:
                ...
                with open(save_path, 'a', encoding='utf-8') as f:
                    for i, generated_answer in enumerate(batched_generated_answer):
                        record = {
                            "answer": generated_answer,
                            "num_tokens": int(token_count),
                            "nfe": int(nfe),
                            "transfer_steps": transfer_steps[i].tolist(),
        ...
        if self.show_speed:
            ...
            if self.save_dir is not None:
                metrics = {
                    ...
                    "tokens_per_second": float(num_tokens / (end_time - start_time)),
